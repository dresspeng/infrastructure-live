locals {
  ...
  modules_git_prefix = format("%s%s",
    local.modules_git_host_auth_method == "ssh" ? "git::git@${local.modules_git_host_name}:" : (
      local.modules_git_host_auth_method == "https" ? (
        local.modules_repository_visibility == "private" ? "git::https://oauth2:${get_env("GITHUB_TOKEN")}@${local.modules_git_host_name}" : "git::https://${local.modules_git_host_name}"
      ) : null
    ),
